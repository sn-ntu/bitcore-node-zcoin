...
To develop bitcore-node-zcoin:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-zcoin.git
...
We now will setup symlinks in `bitcore-node-zcoin` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-zcoin
ln -s ~/bitcoind-rpc-zcoin
```
...
cd devnode
mkdir node_modules
touch bitcore-node-zcoin.json
touch package.json
```

Edit `bitcore-node-zcoin.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-zcoin/bin/bitcore-node-zcoin start
```